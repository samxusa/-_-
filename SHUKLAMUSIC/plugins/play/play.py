...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if document:
            file_name = getattr(video_telegram, "file_name", None) or ""
            ext = file_name.rsplit(".", 1)[-1].lower() if "." in file_name else ""
            # A video MIME type is enough when Telegram omitted the filename
            # extension. Only reject a document when it has a known, invalid
            # extension.
